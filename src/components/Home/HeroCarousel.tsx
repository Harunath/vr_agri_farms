				...
				{carouselImages.map((src, index) => (
					<div key={index} className="relative h-[calc(100vh/2)] lg:h-screen">
						<Image
							...
							height={1080}
							width={1920}
							className="h-full w-screen object-cover opacity-90"
							priority={index === 0}
						/>
					</div>
				))}
...
<motion.div
	...
	viewport={{ once: true }}
	className="absolute top-1/4 lg:top-[30%] left-0 md:left-12 lg:left-24 ml-6 sm:ml-12 p-4 sm:p-8 max-w-xl w-full"
>
				...
				<Link
					href="/about"
					className="inline-block bg-orange-500 text-white font-medium px-6 py-3 rounded-md transition duration-300">
					Learn More
				</Link>
			</motion.div>
